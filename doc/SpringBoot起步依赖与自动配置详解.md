...
本项目 `tlias-web-management` 的 pom.xml 中包含多个 Spring Boot Starter 依赖：
...
引入 `spring-boot-starter-web` → Maven 递归解析 → 同时引入 `spring-webmvc`、`tomcat-embed-core`、`jackson-databind`、`spring-boot-starter` 等十几个依赖。
...
例如：
- classpath 中有 `DataSource` 相关类 + `application.yaml` 配置了数据库连接信息 → 自动创建 `DataSource`、`SqlSessionFactory`、`SqlSessionTemplate` 等 Bean
...
| `application.yaml` 中配置 `spring.autoconfigure.exclude` | 通过配置文件排除 |
...
| 5 | `filter()` | 应用 `@Conditional` 条件过滤（延时执行） |
...
在 `tlias-log-spring-boot-autoconfigure` 模块中创建 SPI 文件。
...
这些自动配置类协同工作，使得这个原本需要大量 XML 配置的项目仅靠 `application.yaml` 中的 27 行配置就能完整运行。
